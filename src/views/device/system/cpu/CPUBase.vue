    ...
    <v-flex xs12>
      <v-card class="elevation-3 mt-2">
        <v-card-title class="pa-2">
          <h6 class="subheading">CPU utilization the Last {{ period }}</h6>
        </v-card-title>
  ...
  methods:
    {
      async fetchData() {
        try {
          this.info = await WanosApi.getCPUData(this.activeOrg._id, this.activeDevice._id, this.period.toLowerCase());
          this.$nextTick(() => {
